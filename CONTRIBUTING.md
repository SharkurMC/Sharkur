Contributing to Sharkur
==========================
Sharkur is happy you're willing to contribute to our projects. We are usually
very lenient with all submitted PRs, but there are still some guidelines you
can follow to make the approval process go more smoothly.

## Use a Personal Fork and not Organization

Sharkur will routinely modify your PR, whether it's a quick rebase or to take care
...
Unfortunately, if you use an organization for your PR, it prevents Sharkur from
...
- A Java 16 or later JDK (packages vary, use Google/DuckDuckGo/etc.).
    ...
    - Sharkur requires JDK 16 to build, however makes use of Gradle's
...
Sharkur is mostly patches and extensions to Purpur/Paper/Spigot. These patches/extensions are
split into different directories which target certain parts of the code. These
directories are:

- `Sharkur-API` - Modifications to `Purpur-API`;
- `Sharkur-Server` - Modifications to `Sharkur`.
...
3. cd into `Sharkur-Server` for server changes, and `Sharkur-API` for API changes.
...
`Sharkur-Server` and `Sharkur-API` aren't git repositories in the traditional sense:

- `base` points to the unmodified source before Sharkur patches have been applied.
- Each commit after `base` is a patch.

## Adding Patches

Adding patches to Sharkur is very simple:

1. Modify `Sharkur-Server` and/or `Sharkur-API` with the appropriate changes;
...
Your commit will be converted into a patch that you can then PR into Sharkur.
...
These steps assume the `origin` remote is your fork of this repository and `upstream` is the official Sharkur repository.
...
1. If your PR creates new patches instead of modifying exist ones, in both the `Sharkur-Server` and `Sharkur-API` directories, ensure your newly-created patch is the last commit by either:
...
Remember that these changes will affect everyone who runs Sharkur, not just you
and your server.
...
All modifications to non-Sharkur files should be marked.

- Multi-line changes start with `// Sharkur start` and end with `// Sharkur end`;
- You can put a comment with an explanation if it isn't obvious, like this:
  `// Sharkur start - reason`.
    - The comments should generally be about the reason the change was made, what
      it was before, or what the change is.
    - Multi-line messages should start with `// Sharkur start` and use `/* Multi
      line message here */` for the message itself.
- One-line changes should have `// Sharkur` or `// Sharkur - reason`.

Here's an example of how to mark changes by Sharkur:

```java
entity.getWorld().dontbeStupid(); // Sharkur - was beStupid() which is bad
entity.getFriends().forEach(Entity::explode);
entity.a();
entity.b();
// Sharkur start - use plugin-set spawn
...
When submitting patches to Sharkur, we may ask you to add notes to the patch
...
If you add a message to your commit in the `Sharkur-Server`/`Sharkur-API`
...
to make future updates easier on ourselves, Sharkur tries to use obfuscation
...
```java
double d0 = entity.getX(); final double fromX = d0; // Sharkur - OBFHELPER
// ...
this.someMethod(fromX); // Sharkur
```
...
`SharkurConfig` classes. Use `SharkurConfig` if a value
must remain the same throughout all worlds.

### SharkurConfig example
...
```java
if (!SharkurConfig.saveEmptyScoreboardTeams) {
```

It is often preferred that you use the fully qualified name for the
configuration class when accessing it, like so:
`org.Sharkurmc.Sharkur.SharkurConfig.valueHere`.
If this is not done, a developer for Sharkur might fix that for you before
merging, but it's always nice if you make it a habit where you only need 1-2
lines changed.

## Testing API changes

### Using the Sharkur Test Plugin

The Sharkur project has a `test-plugin` module for easily testing out API changes
...
of the other Sharkur run tasks).

### Publishing to Maven local (use in external plugins)

To build and install the Sharkur APIs and Server to your local Maven repository, do the following:
...
- Add `mavenLocal()` as a repository. Gradle checks repositories in the order they are declared,
  so if you also have the Sharkur repository added, put the local repository above Sharkur's.
...
- If you later need to use the Sharkur-API, you might want to remove the jar
...
By default, Sharkur (and upstream) only import files we make changes to. If you
would like to make changes to a file that isn't present in `Sharkur-Server`'s
...
1. Identify the name(s) of the file(s) you want to import.
    - A complete list of all possible file names can be found at
      `./Sharkur-Server/.gradle/caches/paperweight/mc-dev-sources/net/minecraft/`. You might find
